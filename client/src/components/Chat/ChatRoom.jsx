  ...
  return (
    <div className="flex h-[100dvh]" style={{ backgroundColor: '#1a1a2e', padding: '16px' }}>
      <div className="flex w-full max-w-[1600px] mx-auto rounded-[var(--border-radius-lg)] overflow-hidden shadow-2xl" style={{ backgroundColor: 'var(--bg-primary)' }}>
        ...
        <div className={`flex ${mobileView === 'list' ? 'flex' : 'hidden'} md:flex w-full md:w-[var(--sidebar-width)] flex-shrink-0 min-h-0`}>
          <div className="hidden md:flex">
            <IconBar />
          </div>
          <ChatListSidebar onRoomOpen={() => setMobileView('chat')} />
        </div>

        <div className={`flex flex-col flex-1 min-h-0 ${mobileView === 'chat' ? 'flex' : 'hidden'} md:flex`}>
          <div className="px-6 py-4 flex items-center justify-between border-b border-[var(--bg-card)]" style={{ backgroundColor: 'var(--bg-primary)' }}>
            <div className="flex items-center gap-4">
              <button
                onClick={handleBack}
                className="md:hidden h-11 w-11 text-[var(--text-secondary)] hover:text-white transition-colors flex items-center justify-center"
              >
            ...
            <div className="flex items-center gap-2">
              <button className="hidden md:flex w-10 h-10 rounded-xl bg-[var(--bg-card)] items-center justify-center text-[var(--text-secondary)] hover:text-white transition-colors">
              ...
              <button className="w-11 h-11 rounded-xl bg-[var(--bg-card)] flex items-center justify-center text-[var(--text-secondary)] hover:text-white transition-colors">
